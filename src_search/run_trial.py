...
Each call:
  1. Load search spec yaml (defines base_yaml + which dims to sample)
  2. Sample yaml params from spec (deterministic given --seed)
  3. Write trial_yaml.yaml + params.json into trial folder
  4. Run search_trainer with that yaml
  5. summary.json + epoch_log.jsonl land in the same folder
"""
...
def main():
    ...
    parser.add_argument('--search_spec', type=str, required=True,
                        help='Path to search spec yaml. Defines base_yaml and '
                             'which dims to sample (with their ranges).')
    ...
    spec = load_spec(args.search_spec)
    base_yaml_path = resolve_base_yaml(spec, PROJECT_ROOT)
