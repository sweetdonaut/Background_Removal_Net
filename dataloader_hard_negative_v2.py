...
class Dataset(Dataset):
    ...
    Also includes hard negative samples (real stripe images) with all-zero masks
    Uses systematic sliding window to ensure all parts of images are used

    Hard negative configuration is built-in for simple A/B testing
    """
    ...
    def __init__(self, training_path, patch_size=(256, 256),
        ...
        # Built-in hard negative configuration
        # Modify these values directly to change behavior
        self.hard_negative_ratio = 0.2  # 20% of samples will be hard negatives
        # Set hard negative path relative to training path (same level as good folder)
        # e.g., if training_path is ./MVTec_AD_dataset/grid_stripe/train/good/
        # then hard_negative_path will be ./MVTec_AD_dataset/grid_stripe/train/hard_negative_samples/
        training_dir = os.path.dirname(training_path.rstrip('/'))  # Go up one level from good
        self.hard_negative_path = os.path.join(training_dir, "hard_negative_samples")
        ...
        if os.path.exists(self.hard_negative_path):
            # Load hard negative images
            if img_format == 'png_jpg':
                self.hard_negative_paths = glob(os.path.join(self.hard_negative_path, "*.png"))
                self.hard_negative_paths.extend(glob(os.path.join(self.hard_negative_path, "*.jpg")))
            elif img_format == 'tiff':
                self.hard_negative_paths = glob(os.path.join(self.hard_negative_path, "*.tiff"))
                self.hard_negative_paths.extend(glob(os.path.join(self.hard_negative_path, "*.tif")))
            ...
            if len(self.hard_negative_paths) > 0:
                ...
                print(f"Hard negative ratio: {self.hard_negative_ratio} (built-in)")
            else:
                print(f"Warning: No hard negative images found in {self.hard_negative_path}")
                print("Training will proceed without hard negatives")
        else:
            print(f"Hard negative path not found: {self.hard_negative_path}")
            print("Training will proceed without hard negatives")
    ...
    def _get_hard_negative_sample(self, idx):
        ...
        # For hard negatives, use the original three channels directly
        # These images already have natural channel differences that cause false positives
        # We want the model to learn that these patterns should NOT be detected as defects
        target = patch[:, :, 0].copy()
